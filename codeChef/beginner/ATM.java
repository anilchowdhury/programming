...
/**
 ...
 (including bank charges). For each successful withdrawal the bank charges 0.50 $US. Calculate Pooja's account balance
...
class ATM {
    ...
    public static void main(String[] args) {
        ...
        if (argLength != 2) {
            System.out.println("Usage : <amount to withdraw> <balance>");
            System.exit(0);
        }

        double amountToWithDraw = Integer.parseInt(args[0]);
        double accountBalance = Integer.parseInt(args[1]);

        if (accountBalance < (amountToWithDraw + charges)) {
            System.out.println(accountBalance);
        } else if (amountToWithDraw % 5 != 0) {
            System.out.println(accountBalance);
        } else {
            accountBalance = accountBalance - amountToWithDraw - charges;
            System.out.println(accountBalance);
        }
    }
}
